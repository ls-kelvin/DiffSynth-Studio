...
class WanVideoAutoregressiveInterPipeline(WanVideoInterPipeline):
    ...
    def _build_mllm_mask(
        ...
        mllm_mask = torch.full((1, num_dit_tokens), 0, device=input_ids.device, dtype=torch.int32)
        ...
        for block_idx, block in enumerate(block_info):
            ...
            elif len(vision_end_positions) == 0 or len(vision_start_positions) == 0:
                mllm_mask[:, start_dit_token:end_dit_token] = 0
            else:
    ...
    def encode_mllm_for_block(
        ...
        mllm_mask, mllm_kv_len, mllm_vision_mask = self._build_mllm_mask(
    ...
    def denoise_block(
        ...
        mllm_vision_mask: Optional[torch.Tensor],
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(
            ...
            if w_t != 0.0 or w_m != 0.0:
                ...
                if w_m != 0.0:
                    noise_pred_text_only = compute_noise_pred_per_block(
                        dit=dit,
                        block_idx=block["global_block_idx"],
                        block_info=block,
                        x_full=full_latents,
        ...
        for block in block_info:
            ...
            if use_mllm_condition and block_idx > 0:
                ...
                if (
                    ...
                    mllm_vision_mask = mllm_output.get("mllm_vision_mask")
