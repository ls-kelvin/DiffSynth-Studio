    ...
    **kwargs,
    ...
    if input_latents is not None and num_blocks > 1:
        ...
        def get_clean_tokens(seq_frame_idx: int):
            ...
            clean_camera = control_camera_latents_input[:, :, clean_frame_idx:clean_frame_idx + 1] if control_camera_latents_input is not None else None
            clean_tokens = dit.patchify(clean_latent, clean_camera)
        ...
        for block_idx in range(num_blocks):
            ...
            if block_idx > 0:
                ...
                keep_mask_segments.append(torch.zeros((seg_len,), device=x.device, dtype=torch.bool))
            ...
            block_id_segments.append(torch.full((seg_len,), block_idx, device=x.device, dtype=torch.int32))
            keep_mask_segments.append(torch.ones((seg_len,), device=x.device, dtype=torch.bool))
        ...
        if FLEX_ATTENTION_AVAILABLE and create_block_mask is not None and x is not None:
            ...
            if keep_mask is not None:
                ...
                for block_idx in range(num_blocks):
                    ...
                    if block_idx > 0:
                        block_ids_list.append(torch.full((tokens_per_latent_frame,), block_idx, device=x.device, dtype=torch.int32))
            ...
            def mask_mod(b, h, q_idx, kv_idx):
                return block_ids[q_idx] == block_ids[kv_idx]
