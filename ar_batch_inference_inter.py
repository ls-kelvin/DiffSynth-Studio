...
from diffsynth.pipelines.wan_video_autoregressive_inter_2 import (
...
def load_lora_weights(pipe, lora_path, device, rank=0):
    ...
    Args:
        pipe: WanVideoAutoregressiveInterPipeline实例
...
def main():
    ...
    for idx in indices:
        ...
        try:
            if hasattr(pipe, "dit") and hasattr(pipe.dit, "reset_cross_attn_norm_stats"):
                pipe.dit.reset_cross_attn_norm_stats()
            output_video = pipe(
            ...
            if hasattr(pipe, "dit") and hasattr(pipe.dit, "get_cross_attn_norm_stats"):
                norm_stats = pipe.dit.get_cross_attn_norm_stats()
                stats_filename = f"{video_id}_ar_inter_cross_attn_norms.json"
                stats_save_path = os.path.join(output_dir, stats_filename)
                counter = 1
                orig_stats_save_path = stats_save_path
                while os.path.exists(stats_save_path):
                    name, ext = os.path.splitext(orig_stats_save_path)
                    stats_save_path = f"{name}_{counter}{ext}"
                    counter += 1
                with open(stats_save_path, "w", encoding="utf-8") as f:
                    json.dump(
                        {
                            "video_id": video_id,
                            "num_records": len(norm_stats),
                            "records": norm_stats,
                        },
                        f,
                        ensure_ascii=False,
                ...
                accelerator.print(f"✅ [Rank {rank}] Saved cross-attn norms: {stats_save_path}")
