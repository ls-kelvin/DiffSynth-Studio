...
accelerate launch \
  ...
  examples/wanvideo/model_training/train_clean_vae.py \
  --dataset_base_path "" \
  ...
  --output_path "./models/train2/Wan2.1-T2V-1.3B_lora_agibot-alpha_wo_mllm" \
  --task "sft" \
  --lora_base_model "dit" \
  ...
  --use_wandb \
  --wandb_project "SSD" \
  --wandb_run_name "wan2.1-1.3b-t2v_agibot-alpha_wo_mllm" \
  --save_steps 800 \
  --t5_cfg_drop 0.1