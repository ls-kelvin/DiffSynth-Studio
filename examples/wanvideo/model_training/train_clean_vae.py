...
from diffsynth.pipelines.wan_video_clean_vae import WanVideoCleanVAEPipeline, ModelConfig
from diffsynth.diffusion import *
import random
random.seed(42)
os.environ["TOKENIZERS_PARALLELISM"] = "false"

# Global stride variable for latent frame sampling
...
class WanCleanVAETrainingModule(DiffusionTrainingModule):
    def __init__(
        self,
        ...
        if not use_gradient_checkpointing:
            warnings.warn("Gradient checkpointing is detected as disabled. Enabling it to avoid OOM.")
            use_gradient_checkpointing = True
        model_configs = self.parse_model_configs(model_paths, model_id_with_origin_paths, fp8_models=fp8_models, offload_models=offload_models, device=device)
        tokenizer_config = ModelConfig(path=tokenizer_path)
        self.pipe = WanVideoCleanVAEPipeline.from_pretrained(
            torch_dtype=torch.bfloat16,
            device=device,
            model_configs=model_configs,
            tokenizer_config=tokenizer_config,
            mllm_processor_config=None,  # No MLLM
        )
    ...
    def get_pipeline_inputs(self, data):
        ...
        inputs_shared = {
            ...
            "use_mllm_condition": False,  # No MLLM
...
def wan_parser():
    parser = argparse.ArgumentParser(description="Training script for clean VAE history Wan.")
    ...
    parser.add_argument("--latent_stride", type=int, default=4, help="Stride for latent frame sampling.")
    return parser
...
if __name__ == "__main__":
    ...
    # Use global LATENT_STRIDE if provided in args
    latent_stride = args.latent_stride if hasattr(args, 'latent_stride') else LATENT_STRIDE
    
    model = WanCleanVAETrainingModule(
